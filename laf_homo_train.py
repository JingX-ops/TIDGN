...
train_features_dir = './Feature/LAF in LAF-LAF/feature_train'
train_edges_dir = './Contactmap/LAF in LAF-LAF/distance_train'
test_features_dir = './Feature/LAF in LAF-LAF/feature_test'
test_edges_dir = './Contactmap/LAF in LAF-LAF/distance_test'
label_file = './Dataset/LAF in LAF-LAF/test_laf_homo.txt'

# 加载预训练的GCNLSTM模型
model_path = './Model/LAF in LAF-LAF/Pretrain_laf_c.pth'
gcn_lstm_model = GCNLSTM(node_features=25, hidden_dim=168)
gcn_lstm_model.load_state_dict(torch.load(model_path))
gcn_lstm_model.eval()  # 设置为评估模式
...
def load_real_data(features_dir, edges_dir, i, labels):
    ...
    for j in range(1, 11):
        ...
        edge_data = np.loadtxt(edge_file_path, delimiter=',', usecols=(0, 1)).astype(int) - 2

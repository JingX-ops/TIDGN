...
class GCNLSTM(nn.Module):
    def __init__(self, node_features, hidden_dim, num_classes=73, dropout_rate=0.5):
        super(GCNLSTM, self).__init__()
        self.recurrent = GConvLSTM(in_channels=node_features, out_channels=hidden_dim)
        self.dropout = nn.Dropout(dropout_rate)
        self.MLP = nn.Sequential(
            nn.Linear(hidden_dim * 73, hidden_dim // 4),
            nn.ReLU(),
...
train_features_dir = './Feature/FUS in FUS-FUS/feature_train'
train_edges_dir = './Contactmap/FUS in FUS-FUS/distance_train'
test_features_dir = './Feature/FUS in FUS-FUS/feature_test'
test_edges_dir = './Contactmap/FUS in FUS-FUS/distance_test'
label_file = './Dataset/FUS in FUS-FUS/test_fus_homo.txt'

model_path = './Model/FUS in FUS-FUS/Pretrain_fus_c.pth'
gcn_lstm_model = GCNLSTM(node_features=25, hidden_dim=73)
...
class GCN_MLP(nn.Module):
    def __init__(self, gcn_lstm_model):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(73 * 73, 512),
            nn.ReLU(),
            nn.Linear(512, 73)  # Make sure the output is [batch_size, 73]
        )
    
    def forward(self, temporal_data):
        output = self.gcn_lstm(temporal_data)
        output = self.mlp(output)
        return output  # Output shape should be [batch_size, 73]
